...
const fetchBestMove = useCallback(async (currentFen) => {
  // Use the local API. The max depth is handled by the server, but we can cap it here too.
  const depth = Math.min(maxDepth, 15);
  console.log(`[ChessGame] AI Fetch (Local API @ Depth: ${depth})`);
  ...
  try {
    ...
    if (!res.ok) {
      ...
      try {
        errorDetails = JSON.parse(errorText).error || errorText;
      } catch (e) { /* Not a JSON error, use raw text */ }
      
      throw new Error(`Local API error: ${res.status} - ${errorDetails}`);
    }

    const data = await res.json();

    // CORRECTED LOGIC:
    // 1. Your local API response has the move directly in `data.bestmove`.
    // 2. There is no `data.success` key.
    // 3. There is no need to split the string by spaces.
    if (data.bestmove && typeof data.bestmove === 'string') {
      const uciMove = data.bestmove; // e.g., "e2e4" or "a7a8q"

      // This part was already correct for parsing a UCI move string.
      ...
      console.error("[fetchBestMove] Invalid or missing 'bestmove' in response from local API:", data);
      return null;
    }
  } catch (err) {
    console.error("[fetchBestMove] Fetch error:", err.message);
    // You could implement a fallback to the online API here if desired.
    return null;
  }
}, [maxDepth]);
