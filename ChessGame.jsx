...
const ChessGame = ({ initialFen, maxDepth = 15, minDepth = 10 }) => {
  ...
  //   const url = `/api/stockfish/analyze?fen=${encodeURIComponent(currentFen)}&depth=${depth}`;
...
const fetchBestMove = useCallback(async (currentFen) => {
  // Use the local multithreaded API. The max depth is handled by the server, but we can cap it here too.
  const depth = Math.min(maxDepth, 15);
  console.log(`[ChessGame] AI Fetch (Multithreaded API @ Depth: ${depth})`);
  ...
  try {
    ...
    if (!res.ok) {
      const errorText = await res.text();
      console.error(`[fetchBestMove] HTTP ${res.status} response:`, errorText);
      ...
      try {
        ...
        // Not a JSON error, check if it's HTML (likely a 404 page)
        if (errorText.includes('<!doctype') || errorText.includes('<html>')) {
          errorDetails = `Backend service not running or not accessible. Please ensure the Node.js backend is running on port 5000.`;
        } else {
          errorDetails = errorText;
        }
      }
      
      throw new Error(`Multithreaded API error: ${res.status} - ${errorDetails}`);
    }
    ...
    try {
      ...
      console.error("[fetchBestMove] Failed to parse JSON response:", jsonError.message);
      console.error("[fetchBestMove] Response text:", await res.text());
      throw new Error("Invalid JSON response from backend");
    }

    // Enhanced response handling for multithreaded API:
    // 1. Check for success flag first
    if (!data.success) {
      console.error("[fetchBestMove] API returned unsuccessful response:", data);
      return null;
    }

    // 2. Parse the best move from the response
    if (data.bestmove && typeof data.bestmove === 'string') {
      const uciMove = data.bestmove; // e.g., "e2e4" or "a7a8q"
      ...
      // Log multithreaded status for debugging
      if (data.multithreaded) {
        console.log(`[fetchBestMove] ✅ Multithreaded analysis complete: ${uciMove}`);
      }
      
      return { from, to, promotion };

    } else {
      console.error("[fetchBestMove] Invalid or missing 'bestmove' in multithreaded API response:", data);
      return null;
    }
  } catch (err) {
    console.error("[fetchBestMove] Multithreaded API fetch error:", err.message);
    return null;
  }
}, [maxDepth]);
